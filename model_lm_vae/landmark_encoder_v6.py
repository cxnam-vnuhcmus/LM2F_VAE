...
class FusionModel(nn.Module):
    def __init__(self, n_frame=5):
        super(FusionModel, self).__init__()
        self.n_frame = n_frame
        self.spade_layer_1 = SPADE(8, 4*n_frame, 32)
        self.spade_layer_2 = SPADE(8, 4*n_frame, 32)
        self.spade_layer_3 = SPADE(8, 4*n_frame, 32)
        ...
        self.spade_layer_4 = SPADE(4*2*n_frame, 8, 32)
        # self.adain_layer_4 = AdaIN(20, 8)
        self.spade_layer_5 = SPADE(4*2*n_frame, 8, 32)
        # self.adain_layer_5 = AdaIN(20, 8)
        self.spade_layer_6 = SPADE(4*2*n_frame, 4, 32)
    ...
    def forward(self, mask_gt_img_feature, gt_lm_feature, ref_img_feature, ref_lm_feature): #(B,N,4,32,32)
        ...
        x = self.leaky_relu(x)
        x = self.conv_last(x)
        x = x.reshape(target_input.size(0),self.n_frame,4,32,32)
