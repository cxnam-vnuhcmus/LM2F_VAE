...
class Dataset(td.Dataset):
        ...
        for fileline in tqdm(filelists, desc="Loading datas"):
            ...
            for i in range(2, len(lm_paths)-10,5):
                ...
                if os.path.exists(vs_path) and os.path.exists(vs_feat_path) and os.path.exists(vs_mask_feat_path):
                    all_datas.append((lm_path, vs_feat_path, vs_mask_feat_path, vs_path, audio_name, max(0,i-2)))
    ...
    def __getitem__(self, idx):        
        ...
        (lm_path, vs_feat_path, vs_mask_feat_path, vs_path, audio_name, seg_idx) = self.all_datas[idx]
        ...
        return (lm_feature, gt_img_feature, gt_imgs, vs_paths, None)
    ...
    def collate_fn(self, batch):
        ...
        if not all(img is None for img in batch_mfcc):
            ...
            batch_mfcc = torch.cat(batch_mfcc, dim=0)
        else:
            batch_mfcc = None
