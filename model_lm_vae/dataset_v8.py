...
class Dataset(td.Dataset):
    ...
    def __getitem__(self, idx):        
        ...
        gt_img_features = torch.Tensor(gt_img_features[len(gt_img_features)//2])
        # gt_img_mask_features = torch.cat(gt_img_mask_features, dim=0)
        gt_img_mask_features = torch.Tensor(gt_img_mask_features[len(gt_img_mask_features)//2])
        # mfcc_features = torch.stack(mfcc_features, dim=0)
        # gt_imgs = torch.cat(gt_imgs, dim=0)
        gt_imgs = torch.Tensor(gt_imgs[len(gt_imgs)//2])
